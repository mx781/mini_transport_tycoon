...
let _traveltime_piqi_type = Piqirun_ext.find_piqi_type "gameelements/traveltime"
...
let parse_traveltime ?opts x (format :Piqirun_ext.input_format) =
  let x_pb = Piqirun_ext.convert _traveltime_piqi_type format `pb x ?opts in
  let buf = Piqirun.init_from_string x_pb in
  Gameelements_piqi.parse_traveltime buf
...
let gen_traveltime ?opts x (format :Piqirun_ext.output_format) =
  let buf = Gameelements_piqi.gen_traveltime x in
  let x_pb = Piqirun.to_string buf in
  Piqirun_ext.convert _traveltime_piqi_type `pb format x_pb ?opts
...
let print_traveltime ?opts x =
  Pervasives.print_endline (gen_traveltime x `piq ?opts)
let prerr_traveltime ?opts x =
  Pervasives.prerr_endline (gen_traveltime x `piq ?opts)
