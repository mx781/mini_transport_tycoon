...
module rec GS_piqi:
  sig
    type game_state = Piqirun_custom.game_state
    ...
    type vehiclelist = GS_piqi.vehicle list
    type playerlist = GS_piqi.player list
    type gamestate = Gamestate.t
  end = GS_piqi
and Gamestate:
  sig
    type t = {
      mutable vehicles: GS_piqi.vehiclelist;
      mutable players: GS_piqi.playerlist;
      mutable aiinfo: GS_piqi.funcopt;
      mutable gameage: int;
      mutable paused: bool;
      mutable graph: GS_piqi.graph;
    }
  end = Gamestate
...
and parse_game_state x = (Piqirun_custom.game_state_of_gamestate(parse_gamestate x))
...
and gen__game_state code x = gen__gamestate code(Piqirun_custom.game_state_to_gamestate( x))
...
and gen__funcopt code (x:GS_piqi.funcopt) =
...
let gen_game_state x = gen__game_state (-1) x
...
and default_game_state () = (Piqirun_custom.game_state_of_gamestate(default_gamestate ()))
